...
describe('ts-unios', () => {
  ...
  describe('filter', () => {
    ...
    test('should filter by single type and predicate', () => {
      const result = helpers.filter(values, {
        bType: (b) => b.data > 10
      });

      expect(result).toHaveLength(1);
      expect(result[0]).toEqual({ type: 'bType', data: 15 });
    });
  ...
  describe('narrow', () => {
    ...
    test('should narrow to single type', () => {
      const result = helpers.narrow(values, 'aType');
    ...
    test('should narrow to multiple types', () => {
      const result = helpers.narrow(values, ['aType', 'bType']);

      expect(result).toHaveLength(4);
      expect(result[0].type).toBe('aType');
      expect(result[1].type).toBe('bType');
    });

    test('should return empty array if type not found', () => {
      const result = helpers.narrow([], 'aType');
  ...
  describe('Complex state transitions', () => {
    ...
    test('should handle state transitions', () => {
      ...
      const completedStates = stateHelpers.filter(states, {
        success: () => true,
        error: () => true
      });
